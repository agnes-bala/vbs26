...
import {
  ...
  getCitiesData,
  getStatesData,
} from "../../../services/JRMLookupService";
...
export default function ContactFormPopup(field) {
  ...
  const userData = async () => {
    axios
      .get(`${url}jrms/v1/partners/${pId}/contactinfo`, { headers: configure })
      .then((response) => {
  ...
  const getStateList = async () => {
    await axios
      .get(`${url}jrms/v1/lookup/states?country=${getCountry}`)
      .then((response) => {
  ...
  const handleState = async (event, value) => {
    ...
    const result =
      dist.data !== undefined
        ? dist.data.districtList.filter(
            (d, i, ar) => d !== "''" && d !== null && ar.indexOf(d) === i
          )
        : [];
  ...
  const RegisterSchema = Yup.object().shape({
    ...
    district: Yup.string().when("country", {
      is: (value) => value && value === "India",
      then: Yup.string().required("district is required").max(50).nullable(),
    ...
    pincode: Yup.mixed()
      ...
      .when("country", {
        ...
        then: Yup.number()
          ...
          .min(110000, "Invaid pincode")
          .max(859999, "Invaid pincode")
  ...
  const formik = useFormik({
    ...
    onSubmit: async (contactInfo) => {
      ...
      const result = await updatePartnerContactInfo(
        authInfo.profileUrl,
        authInfo.jwt,
        contactInfo
      );
      if (!result.ok) {
        ...
        return toast.error(
          "Could not update Personal ContactInfo to server ",
          result.data.message
        );
      }
      try {
            ...
            <Stack spacing={3}>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                <FormControl sx={{ minWidth: 120 }}>
                  ...
                  <InputLabel
                    id="title"
                    color={touched.title && errors.title ? "error" : "primary"}
                  >
                    Title
                  </InputLabel>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                <FormControl sx={{ minWidth: "50%" }}>
                  <Select
                    ...
                    name="selectedGender"
                    onChange={(e) => {
                      formik.setFieldValue("selectedGender", e);
                    }}
                  ...
                  <InputLabel
                    id="gender"
                    color={
                      touched.gender && errors.gender ? "error" : "primary"
                    }
                  >
                    Gender
                  </InputLabel>
            ...
            <Stack spacing={3}>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                ...
                {/* <FormControl sx={{ minWidth: "50%" }}>
                  ...
                  >
                    <MenuItem key={getCountry} value={getCountry}>
                      {getCountry}
                    </MenuItem>
                  </Select>
                  <InputLabel
                    id="country"
                    color={
                      touched.country && errors.country ? "error" : "primary"
                    }
                  >
                    Country
                  </InputLabel>
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        {...getFieldProps("stateInAddress")}
                        error={Boolean(
                          touched.stateInAddress && errors.stateInAddress
                        )}
                        helperText={
                          touched.stateInAddress && errors.stateInAddress
                        }
