...
import {
  Grid, Stack, TextField, Button, Divider, FormControl, FormLabel, FormHelperText,
  RadioGroup, FormControlLabel, Radio, MenuItem, Typography
} from '@mui/material';
import { LoadingButton } from '@mui/lab';
import { MobileDatePicker } from '@mui/x-date-pickers';
...
export default function ChildForm() {
    ...
    children: Yup.array().of(
      Yup.object().shape({
        ...
        dateOfBirth: Yup.string()
          .nullable()
          .test("dateOfBirth", "You must be 15 years or younger", value => {
            if (!value) return true;
            return moment().diff(moment(value, "YYYY-MM-DD"), "years") < 15;
          })
          .test("dateOfBirth", "Child age must be 5 months or more", value => {
            if (!value) return true;
            return moment().diff(moment(value, "YYYY-MM-DD"), "months") > 5;
          })
  ...
  const formik = useFormik({
    ...
    onSubmit: async (childrenInfo) => {
      try {
        ...
        console.error("Error submitting children info:", error);
        toast.error("An error occurred while saving children information");
      }
    },
  });
  ...
  useEffect(() => {
    const fetchChildrenData = async () => {
      try {
        ...
        if (!pId || !token) {
          console.error("Missing partnerId or token");
          setIsLoading(false);
          return;
        }
        
        const config = { 'Authorization': `Bearer ${token}` };
        const response = await axios.get(`${url}jrms/v1/partners/${pId}/childreninfo`, { headers: config });
        
        if (response.data) {
          setChilddata(response.data);
          // Don't use setFieldValue here as formik might not be initialized yet
          // The enableReinitialize will handle updating form values
        }
      } catch (err) {
  ...
  // Update child count whenever children array changes
  useEffect(() => {
    if (values.children) {
  ...
  const handleDateChange = (value, fieldName) => {
    if (value === null) {
      ...
      setFieldValue(fieldName, moment(value).format('YYYY/MM/DD'));
    }
  };
                            ...
                            <div key={index}>
                              <Stack spacing={3}>
                                ...
                                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                                  ...
                                  <FormControl>
                                    <Button
                                      sx={{ minWidth: '200px', mt: { xs: 2, sm: 0 } }}
                                      color="error"
                                      size="small"
                                      variant="outlined"
                        ...
                        {childCount < 10 && (
                          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
                            <Button
                              ...
                              disabled={formik.values.relationship < 2 && childCount > 11}
                              onClick={() =>
                                push({ childId: "", fullName: "", dateOfBirth: "", gender: "" })
                              }
                            >
                              Add Child +
                            </Button>
