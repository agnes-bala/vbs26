  ...
  "&::before": {
    ...
    background:
      theme.palette.mode === "dark"
        ? `linear-gradient(135deg, ${alpha("#000", 0.7)} 0%, ${alpha(
            "#1A1A2E",
            0.8,
          )} 100%)`
        : `linear-gradient(135deg, ${alpha("#FFF0F5", 0.3)} 0%, ${alpha(
            "#E6E6FA",
            0.4,
          )} 100%)`,
    zIndex: 0,
...
const getEventStatus = () => {
  ...
  if (currentDateTime.isBefore(eventStartDate)) {
    ...
    currentDateTime.isSameOrAfter(eventStartDate) &&
    currentDateTime.isBefore(eventEndDate)
  ) {
    return "ongoing";
  } else {
    return "ended";
  ...
  boxShadow:
    theme.palette.mode === "dark"
      ...
      : `0 20px 40px ${alpha(theme.palette.violet?.[900] || "#311B92", 0.15)},
       0 0 0 1px ${alpha(
         theme.palette.violet?.[300] || "#9575CD",
         0.15,
       )} inset`,
  padding: theme.spacing(3),
  [theme.breakpoints.up("md")]: {
...
const ProfileWarningCard = styled(Paper)(({ theme }) => ({
  padding: theme.spacing(4),
  ...
  boxShadow:
    theme.palette.mode === "dark"
      ...
      : `0 8px 32px ${alpha(theme.palette.warning.main, 0.1)}`,
  textAlign: "center",
  ...
  "@keyframes pulse": {
    ...
    "50%": {
      boxShadow:
        theme.palette.mode === "dark"
          ? `0 8px 48px ${alpha(theme.palette.warning.main, 0.3)}`
          : `0 8px 48px ${alpha(theme.palette.warning.main, 0.2)}`,
    },
    "100%": {
      boxShadow:
        theme.palette.mode === "dark"
  ...
  background:
    theme.palette.mode === "dark"
      ? `linear-gradient(135deg, ${
        ...
        } 0%, ${theme.palette.violet?.[400] || "#5C6BC0"} 50%, ${
          theme.palette.common.white
        } 100%)`
      : `linear-gradient(135deg, ${
          theme.palette.violet?.[700] || "#5E35B1"
        } 0%, ${theme.palette.violet?.[500] || "#7E57C2"} 50%, ${
          theme.palette.violet?.[300] || "#B39DDB"
        } 100%)`,
...
const ActionButton = styled(Button)(({ theme }) => ({
  ...
  background:
    theme.palette.mode === "dark"
      ? `linear-gradient(135deg, ${
          theme.palette.violet?.[500] || "#3F51B5"
        } 0%, ${theme.palette.violet?.[400] || "#5C6BC0"} 100%)`
      : `linear-gradient(135deg, ${
          theme.palette.violet?.[700] || "#5E35B1"
        } 0%, ${theme.palette.violet?.[500] || "#7E57C2"} 100%)`,
  color: "white",
  boxShadow:
    theme.palette.mode === "dark"
  ...
  "&:hover": {
    background:
      theme.palette.mode === "dark"
        ? `linear-gradient(135deg, ${
            theme.palette.violet?.[600] || "#3949AB"
          } 0%, ${theme.palette.violet?.[500] || "#3F51B5"} 100%)`
        : `linear-gradient(135deg, ${
            theme.palette.violet?.[800] || "#4527A0"
          } 0%, ${theme.palette.violet?.[600] || "#5E35B1"} 100%)`,
...
const WarningButton = styled(Button)(({ theme }) => ({
  ...
  "&:hover": {
    ...
    transform: "translateY(-2px)",
    boxShadow: `0 12px 24px ${alpha(theme.palette.warning.main, 0.4)}`,
  },
}));
...
const CountdownSection = styled(Box)(({ theme }) => ({
  background:
    theme.palette.mode === "dark"
      ? `linear-gradient(135deg, ${alpha(
          theme.palette.violet?.[900] || "#1A237E",
          0.4,
        )} 0%, ${alpha(theme.palette.violet?.[800] || "#283593", 0.4)} 100%)`
      : `linear-gradient(135deg, ${alpha(
          theme.palette.violet?.[50] || "#F3E5F5",
          0.7,
        )} 0%, ${alpha(theme.palette.violet?.[100] || "#EDE7F6", 0.7)} 100%)`,
  borderRadius: theme.spacing(3),
  padding: theme.spacing(3),
  marginBottom: theme.spacing(4),
  border: `1px solid ${
...
function EventCountdown() {
  ...
  const eventEndDate = moment("2026-05-15 23:59", "YYYY-MM-DD HH:mm").toDate();
  ...
  if (eventStatus === "ended") {
    return (
      <Box>
        <StatusBadge
          status="ended"
          sx={{ textAlign: "center", mx: "auto", display: "table" }}
        >
          Event Ended
        </StatusBadge>
        <Typography
...
const CertificateButton = styled(Button)(({ theme }) => ({
  ...
  background:
    theme.palette.mode === "dark"
      ...
      : `linear-gradient(135deg, ${
          theme.palette.violet?.[600] || "#7E57C2"
        } 0%, ${theme.palette.violet?.[500] || "#9575CD"} 100%)`,
  color: "white",
  width: "100%",
  "&:hover": {
    background:
      theme.palette.mode === "dark"
        ...
        : `linear-gradient(135deg, ${
            theme.palette.violet?.[700] || "#5E35B1"
          } 0%, ${theme.palette.violet?.[600] || "#7E57C2"} 100%)`,
  },
}));
...
export default function GeneralApp1() {
  ...
  // Check if profile is complete and if user is first-time
  useEffect(() => {
    const checkProfileAndChildren = async () => {
      try {
        // First, check if user has children registered
        const response = await axios.get(
          `${url}jrms/v1/partners/${pId}/childreninfo`,
          {
            headers: authConfig,
          },
        );
        ...
        if (children.length === 0) {
          // No children registered - first time user
          setIsFirstTimeUser(true);
          setIsProfileComplete(false);
          toast.warning(
            "Please complete your profile and add your children's details.",
          );
        } else {
          // Has children - profile is complete
          setIsFirstTimeUser(false);
          setIsProfileComplete(true);
          setChildInfo(children);
          // Fetch scores for each child
          children.forEach((child) => getChildScore(child.childId));
        }
      } catch (err) {
        console.error("Error checking profile:", err);

        // If the children endpoint fails, try profile endpoint
        try {
          const profileResponse = await axios.get(
            `${url}jrms/v1/partners/${pId}`,
            {
              headers: authConfig,
            },
          );
          ...
          if (!hasRequiredFields) {
            ...
            // Profile exists but maybe no children yet
            setIsFirstTimeUser(true);
            setIsProfileComplete(false);
            toast.info(
              "Welcome! Please add your children's details to get started.",
            );
          }
        } catch (profileErr) {
          // Both endpoints failed - definitely first time user
          setIsFirstTimeUser(true);
          setIsProfileComplete(false);
  ...
  }, [getchildinfo, isProfileComplete]);
  ...
  const getChildScore = async (childId) => {
    try {
      ...
      const totalScore = result.data.gameScoreList.reduce(
        (a, v) => a + v.score,
        0,
      );
  ...
  const isCertificateAvailable =
    currentDateTime > certificateStartDate &&
  ...
  const handleGame = (id, dateOfBirth) => {
    ...
    if (
      dob.diff(moment("2026-02-26", "YYYY-MM-DD"), "days") === 0 ||
  ...
  return (
    <Page title="Dashboard">
      <RootStyle>
        ...
        >
          <GlassCard>
            {/* Profile Incomplete Warning for First-Time Users */}
            {isFirstTimeUser && (
              <ProfileWarningCard>
                <WarningAmberIcon
                ...
                <Typography
                  ...
                  sx={{ mb: 1 }}
                >
                  To get started, you need to complete your profile information.
                </Typography>
                <Typography
                  variant="body2"
                  color="text.secondary"
                  sx={{ mb: 3 }}
                >
                  Please add your details and register your children to access
                ...
                >
                  <WarningButton
                    ...
                    onClick={() => navigate(PATH_DASHBOARD.user.profile)}
                    size="large"
                  >
                    ...
                    {new Date() < new Date("2026-04-27") ? (
                      <Box>
                        ...
                        <SubtitleText variant="body1">
                          Thank you for registering! Get ready for fun games and
                          activities.
                        </SubtitleText>
                      </Box>
                    ) : new Date() >= new Date("2026-04-27") &&
                        ...
                        <Chip
                          ...
                          sx={{
                            ...
                            background:
                              theme.palette.mode === "dark"
                                ? alpha(
                                    theme.palette.violet?.[800] || "#283593",
                                    0.3,
                                  )
                                : alpha(
                                    theme.palette.violet?.[500] || "#7E57C2",
                                    0.1,
                                  ),
                            color:
                              theme.palette.mode === "dark"
                  ...
                  {childInfo.length > 0 ? (
                            ...
                            <ChildInfo>
                              ...
                              <Stack spacing={1} sx={{ mt: 1 }}>
                                    ...
                                    ...(isGameDisabled && {
                                      ...
                                      "&:hover": {
                                        backgroundColor:
                                          theme.palette.mode === "dark"
                                            ? alpha(
                                                theme.palette.violet?.[900] ||
                                                  "#1A237E",
                                                0.3,
                                              )
                                            : alpha(
                                                theme.palette.violet?.[100] ||
                                                  "#EDE7F6",
                                                0.5,
                                              ),
                                      },
                                ...
                                <GameButton
                                  ...
                                  onClick={() =>
                                    handleGame(data.childId, data.dateOfBirth)
                                  }
                                  disabled={isGameDisabled}
                                  fullWidth
                                  sx={{
                              ...
                              <Box sx={{ mt: 2 }}>
                                <CertificateButton
                                  ...
                                  onClick={() => {
                                    ...
                                    navigate(
                                      PATH_DASHBOARD.general.downloadCert,
                                      {
                                        replace: true,
                                      },
                                    );
                    ...
                    <Box
                      sx={{
                        ...
                        border: `1px dashed ${
                          theme.palette.mode === "dark"
                            ? alpha(
                                theme.palette.violet?.[700] || "#303F9F",
                                0.3,
                              )
                            : alpha(
                                theme.palette.violet?.[300] || "#9575CD",
                    ...
                    >
                      ...
                      <Typography
                        ...
                        sx={{ mb: 3 }}
                      >
                        Add your children's information to start playing games
                        and activities
                      </Typography>
                      <ActionButton
                        variant="contained"
